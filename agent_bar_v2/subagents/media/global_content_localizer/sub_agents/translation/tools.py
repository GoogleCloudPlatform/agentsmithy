...
async def translate_text_with_model(
    ...
    The input text can be can be a local upload available in the state already, a string or a GCS URI to a text file containing the transcript.

    Args:
        source_language (str): Language code for the source language of the text.
        target_language (str): Language code for the target language for the output of the translation.
        text (str): String to be translated if it is provided by the user.
        gcs_uri (str): GCS URI to a text file containing the text to be translated.
        artifact_name (str): Path to a local artifact uploaded by the user.
        model_id (str): `general/nmt` or `general/translation-llm` or custom model id provided by the user indicating which model to use.
        tool_context (ToolContext): Contains the state, including any uploaded files.

    Returns:
      dict: Dictionary indicating status and in case of success the translated text.
    """
    if not text and artifact_name:
        ...
        if txt_part and txt_part.inline_data and txt_part.inline_data.data:
            text = txt_part.inline_data.data.decode("utf-8")

    if not text and gcs_uri:
        try:
    ...
    if not text:
        ...
        if filenames:
            ...
            if txt_part and txt_part.inline_data and txt_part.inline_data.data:
                ...
                return {
                    "status": "error",
                    "message": "Provide the text to be translated or the GCS URI to the text file to be translated.",
                }
        else:
            return {
                "status": "error",
    ...
    if source_language.lower() in language_codes:
        source_language = language_codes[source_language]
    elif source_language not in language_codes.values():
        return {"status": "error", "message": "Provide a valid source language."}

    if target_language.lower() in language_codes:
        target_language = language_codes[target_language]
    elif target_language not in language_codes.values():
        return {"status": "error", "message": "Provide a valid target language."}
    ...
    try:
        ...
        return {"status": "success", "translation": output}
    except Exception:
        return {"status": "error", "message": "Could not translate file."}


async def pic_to_text(
    tool_context: ToolContext, gcs_uri: str = "", artifact_name: str = ""
) -> dict:
    """Detects text in an image file. The image can be provided as a part of the state through a preprocessed upload or a GCS URI.

    Args:
        gcs_uri: GCS path to image file starting with `gs://`
        artifact_name: Path to a local artifact uploaded by the user.
        tool_context: ToolContext,

    Returns:
        Dictionary indicating status and in case of success the string of text detected in image
    """
    content = None
    if artifact_name:  # Artifact locally uploaded.
        ...
        if image_part and image_part.inline_data and image_part.inline_data.data:
            content = image_part.inline_data.data
            print("Captured image data from artifact_name")
    if not content and gcs_uri:  # GCS URI provided for an image file.
        storage_client = storage.Client()
        path_parts = gcs_uri[5:].split("/", 1)
        bucket_name = path_parts[0]
        blob_name = path_parts[1]
        ...
        if blob.name.endswith((".jpg", "jpeg")):
            ...
            return {
                "status": "error",
                "message": "Provide the GCS URI to a valid image file.",
            }
    if not content:
        # Check if an image is locally uploaded with the path added to the state variable.
        filenames = tool_context.state.get("image_file")
        if filenames:
            latest_file = filenames[-1]
            image_part = await tool_context.load_artifact(latest_file)
            if image_part and image_part.inline_data and image_part.inline_data.data:
                content = image_part.inline_data.data
                print("Captured image data from state")
            else:
                return {
                    "status": "error",
                    "message": "Provide the GCS URI to the image file to be translated or upload an image locally.",
                }
        else:
            return {
                "status": "error",
    ...
    try:
        ...
        return {"status": "error", "message": "Could not extract text from image."}
...
async def detect_language(
    ...
    Args:
        gcs_uri (str): The GCS URI containing the text file for which the language will be detected.
        text (str): Text for which the language will be detected.
        tool_context (ToolContext): Contains the state, including any uploaded files.
        artifact_name (str): Path to a local artifact uploaded by the user.

    Returns:
        Dictionary containing detected language as key and confidence as content.
    """
    if not text and artifact_name:
        txt_part = await tool_context.load_artifact(artifact_name)
        if txt_part and txt_part.inline_data and txt_part.inline_data.data:
    ...
    try:
        ...
        for language in response.languages:
            result[language.language_code] = language.confidence
        return {"status": "success", "translation": json.dumps(result)}
    except Exception:
        return {"status": "error", "message": "Could not identify language."}


async def write_results_gcs(
    transcript: str, output_format: str, bucket_name: str, file_name: str = ""
) -> dict:
    """Write the resulting script to GCS.
    Args:
        bucket_name (str): Bucket name where the original media file is stored.
    ...
    Returns:
        dict: Dictionary indicating status and in case of success the GCS URI to the file written to GCS
    """
