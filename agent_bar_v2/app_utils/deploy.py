...
@click.option(
    "--requirements-file",
    default="agent_bar_v2/app_utils/.requirements.txt",
...
def deploy_agent_engine_app(
    ...
    # Load all env vars from the file .env except the GOOGLE_CLOUD_PROJECT
    # Search for .env in current directory or agent_bar_v2 directory
    dotenv_path = ".env"
    if not os.path.exists(dotenv_path):
        dotenv_path = "agent_bar_v2/.env"

    if os.path.exists(dotenv_path):
        dot_env_vars = dotenv_values(dotenv_path)
        for key, value in dot_env_vars.items():
            if key != "GOOGLE_CLOUD_PROJECT" and value is not None:
