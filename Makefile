...
deploy:
	...
	(uv export --no-hashes --no-header --no-dev --no-emit-project --no-annotate > agent_bar_v2/app_utils/.requirements.txt 2>/dev/null || \
	uv export --no-hashes --no-header --no-dev --no-emit-project > agent_bar_v2/app_utils/.requirements.txt) && \
	uv run -m agent_bar_v2.app_utils.deploy \
		--source-packages=./agent_bar_v2 \
		--entrypoint-module=agent_bar_v2.agent_engine_app \
		--entrypoint-object=agent_engine \
		--requirements-file=agent_bar_v2/app_utils/.requirements.txt \
...
eval:
	...
	uv run adk eval ./agent_bar_v2 $${EVALSET:-tests/eval/evalsets/basic.evalset.json} \
